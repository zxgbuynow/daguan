...
var toSearch = function(el, wv, shop) {
  if($('.search-modal').length == 0) {
    ...
    $('.history-list li').on('tap', function() {
      ...
      if(!shop) {
  window.location.href = wv+'?keyword='+key;
       // clicked(wv, keyword);
      } else {
        $.extend(shop, keyword);
        clicked(wv, shop);
  ...
  $(form).submit(function() {
    ...
    if(!shop) {
  window.location.href = wv+'?keyword='+el.value;
    } else {
      $.extend(shop, keyword);
      clicked(wv, shop);
    }
...
$(function() {
  ...
  template.helper('$getDateDiff', function(dateTimeStamp) {
    var minute = 1000 * 60;
    var hour = minute * 60;
    var day = hour * 24;
  ...
  template.helper('$add_server', function(num) {
    return config.imgser+num;
  });
