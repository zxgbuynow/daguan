...
class Menu extends Shop
{
    ...
    public function add($module = 'admin', $pid = '')
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($menu = MenuModel::create($data)) {
                ...
                action_shop_log('menu_add', 'shop_menu', $menu['id'], UID, $details);
        ...
        return ZBuilder::make('form')
            ...
            ->addSelect('role', '角色', '除分中心超级管理员外，拥有该节点权限的角色', RoleModel::where('id', 'neq', 1)->column('id,name'), '', 'multiple')
    ...
    public function edit($id = 0)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (MenuModel::update($data)) {
                ...
                // 记录行为
                $details = '节点ID('.$id.')';
                action_shop_log('menu_edit', 'shop_menu', $id, UID, $details);
    ...
    public function delete($record = [])
    {
        ...
        if (MenuModel::destroy($all_ids)) {
            ...
            action_shop_log('menu_delete', 'shop_menu', $id, UID, $details);
            $this->success('删除成功');
        } else {
            $this->error('删除失败');
        }
    ...
    private function getNestMenu($lists = [], $max_level = 0, $pid = 0, $curr_level = 1)
    {
        ...
        foreach ($lists as $key => $value) {
            if ($value['pid'] == $pid) {
                ...
                $result .= '<a href="'.url('delete', ['id' => $value['id'], 'table' => 'shop_menu']).'" data-toggle="tooltip" data-original-title="删除" class="ajax-get confirm"><i class="list-icon fa fa-times fa-fw"></i></a></div>';
                $result .= '</div>';
    ...
    public function enable($record = [])
    {
        ...
        $menu    = MenuModel::where('id', $id)->find();
        $details = '节点ID('.$id.'),节点标题('.$menu['title'].'),节点链接('.$menu['url_value'].')';
        $this->setStatus('enable', ['menu_enable', 'shop_menu', $id, UID, $details]);
    }
    ...
    public function disable($record = [])
    {
        ...
        $menu    = MenuModel::where('id', $id)->find();
        $details = '节点ID('.$id.'),节点标题('.$menu['title'].'),节点链接('.$menu['url_value'].')';
        $this->setStatus('disable', ['menu_disable', 'shop_menu', $id, UID, $details]);
    }
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        if (false !== MenuModel::where('id', $id)->setField('status', $status)) {
            ...
            // 记录行为日志
            if (!empty($record)) {
                call_user_func_array('action_shop_log', $record);
            }
            $this->success('操作成功');
        } else {
            $this->error('操作失败');
