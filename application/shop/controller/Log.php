...
class Log extends Shop
{
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 排序
        $order = $this->getOrder('shop_log.id desc');
        // 数据列表
        ...
        return ZBuilder::make('table')
            ...
            ->setSearch(['shop_action.title' => '行为名称', 'shop_user.username' => '执行者', 'shop_module.title' => '所属模块']) // 设置搜索框
            ->hideCheckbox()
            ->addColumns([ // 批量添加数据列
                ['id', '编号'],
                ['title', '行为名称'],
            ...
            ->addOrder(['title' => 'shop_action', 'username' => 'shop_user', 'module_title' => 'shop_module.title'])
            ->addFilter(['shop_action.title', 'shop_user.username', 'module_title' => 'shop_module.title'])
    ...
    public function details($id = null)
    {
        ...
        $info = LogModel::getAll(['shop_log.id' => $id]);
        $info = $info[0];
