...
use app\shop\model\CateAccess as CateAccessModel;
...
class Consult extends shop
{
    ...
    public function index()
    {
        ...
        $info = CateAccessModel::where($map)->find();

        $cmap['id'] = array('in',$info['cids']);
        
        // 数据列表
        $data_list = CategoryModel::where($cmap)->order('id desc')->paginate();
        ...
        return ZBuilder::make('table')
            ->setPageTitle('咨询业务管理') // 设置页面标题
            ->setTableName('cms_category') // 设置数据表名
            ->hideCheckbox()
            ->addColumns([ // 批量添加列
                ['id', 'ID'],
                ['title', '业务类型'],
