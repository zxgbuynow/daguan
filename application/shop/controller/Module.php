...
use app\shop\model\Plugin as PluginModel;
use app\shop\model\Menu as MenuModel;
use app\shop\model\Action as ActionModel;
use think\Cache;
use util\Database;
use util\Sql;
use util\File;
...
class Module extends Shop
{
    ...
    public function install($name = '', $confirm = 0)
    {
        ...
        if ($ModuleModel->allowField($allowField)->save()) {
            ...
            // 记录行为
            action_shop_log('module_install', 'shop_module', 0, UID, $module_info['title']);
    ...
    public function uninstall($name = '', $confirm = 0)
    {
        ...
        // 删除授权信息
        if (false === Db::name('shop_access')->where('module', $name)->delete()) {
            $this->error('删除授权信息失败，请重新卸载');
        }

        // 删除行为规则
        if (false === Db::name('shop_action')->where('module', $name)->delete()) {
        ...
        if (ModuleModel::where('name', $name)->delete()) {
            ...
            cache('module_all', null);
            // 记录行为
            action_shop_log('module_uninstall', 'shop_module', 0, UID, $module_info['title']);
    ...
    public function export($name = '')
    {
        ...
        // 检查是否有模块行为信息
        $action = Db::name('shop_action')->where('module', $name)->field('module,name,title,remark,rule,log,status')->select();
        if ($action) {
        ...
        // 记录行为
        action_shop_log('module_export', 'shop_module', 0, UID, $module_info['title']);
...
INFO;
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        if (false !== ModuleModel::where('id', $ids)->setField('status', $status)) {
            // 记录日志
            call_user_func_array('action_shop_log', ['module_'.$type, 'shop_module', 0, UID, $module['title']]);
            $this->success('操作成功');
        } else {
            $this->error('操作失败');
        }
