...
class Shop extends Common
{
        ...
        if (!$this->request->isAjax()) {
            ...
            $settings = [
                [
                    'title'   => '站点开关',
                    'tips'    => '站点关闭后将不能访问',
                    'checked' => Db::name('shop_config')->where('id', 1)->value('value'),
                    'table'   => $table_token,
                    'id'      => 1,
                    'field'   => 'value'
                ]
    ...
    {
        // 判断是否登录
        if ($uid = is_shop_signin()) {
            // 已登录
            return $uid;
        } else {
            // 未登录
            $this->redirect('publics/signin');
        }
    }
    ...
    public function quickEdit($record = [])
    {
        ...
        $protect_table = [
            '__SHOP_USER__',
            '__SHOP_ROLE__',
            config('database.prefix').'shop_user',
            config('database.prefix').'shop_role',
        ];
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        $protect_table = [
            ...
            config('database.prefix').'shop_user',
            config('database.prefix').'shop_role',
            config('database.prefix').'shop_module',
        ];
