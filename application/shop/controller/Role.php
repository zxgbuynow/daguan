...
class Role extends shop
{
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($role = RoleModel::create($data)) {
                // 记录行为
                action_shop_log('role_add', 'shop_role', $role['id'], UID, $data['name']);
    ...
    public function edit($id = null)
    {
        ...
        if ($id == 1) $this->error('分中心超级管理员不可修改');
        ...
        if ($this->request->isPost()) {
            ...
            if (RoleModel::update($data)) {
                role_auth();
                // 记录行为
                action_shop_log('role_edit', 'shop_role', $id, UID, $data['name']);
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        return parent::setStatus($type, ['role_'.$type, 'shop_role', $role_id, UID, implode('、', $ids)]);
    }
    ...
    public function quickEdit($record = [])
    {
        ...
        return parent::quickEdit(['role_edit', 'shop_role', $id, UID, $details]);
    }
}
