...
use app\shop\model\Attachment as AttachmentModel;
use think\Image;
use think\File;
use think\Hook;
use think\Db;
    ...
    public function index()
    {
        ...
        return ZBuilder::make('table')
            ...
            ->addColumns([ // 批量添加数据列
                ...
                ['name', '名称'],
                ['size', '大小', 'byte'],
                ['driver', '上传驱动', parse_attr(Db::name('shop_config')->where('name', 'upload_driver')->value('options'))],
...
//                break;
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        return parent::setStatus($type, ['attachment_'.$type, 'shop_attachment', 0, UID, $ids]);
    }
    ...
    public function delete($ids = '')
    {
        ...
        if (AttachmentModel::where('id', 'in', $ids)->delete()) {
            // 记录行为
            $ids = is_array($ids) ? implode(',', $ids) : $ids;
            action_shop_log('attachment_delete', 'shop_attachment', 0, UID, $ids);
            $this->success('删除成功');
        } else {
            $this->error('删除失败');
        }
    ...
    {
        $id = input('post.pk', '');
        return parent::quickEdit(['attachment_edit', 'shop_attachment', 0, UID, $id]);
    }
}