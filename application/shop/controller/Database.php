...
class Database extends Shop
{
    ...
    public function export($ids = null, $start = 0)
    {
        ...
        if ($this->request->isPost() && !empty($tables) && is_array($tables)) {
            ...
            if(false !== $Database->create()){
                ...
                unlink($lock);
                // 记录行为
                action_shop_log('database_export', 'database', 0, UID, implode(',', $tables));
                $this->success('备份完成！');
            } else {
    ...
    public function import($time = 0)
    {
        ...
        if(count($list) === $last[0]){
            ...
            // 记录行为
            action_shop_log('database_import', 'database', 0, UID, date('Ymd-His', $time));
            $this->success('还原完成！');
        } else {
    ...
    public function optimize($ids = null)
    {
        ...
        if($tables) {
            if(is_array($tables)){
                ...
                if($list){
                    // 记录行为
                    action_shop_log('database_optimize', 'database', 0, UID, "`{$tables}`");
    ...
    public function repair($ids = null)
    {
        ...
        if($tables) {
            if(is_array($tables)){
                ...
                if($list){
                    // 记录行为
                    action_shop_log('database_repair', 'database', 0, UID, "`{$tables}`");
    ...
    public function delete($ids = 0)
    {
        ...
        if(count(glob($path))){
            ...
            // 记录行为
            action_shop_log('database_backup_delete', 'database', 0, UID, date('Ymd-His', $ids));
