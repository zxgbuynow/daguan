...
class Menu extends Model
{
    ...
    public static function getTopMenu($max = '', $cache_tag = '')
    {
        ...
        if (!$menus) {
            ...
            foreach ($menus as $key => &$menu) {
                ...
                if ($menu['url_value'] != '' && ($menu['url_type'] == 'module_shop' || $menu['url_type'] == 'module_home')) {
                    $url = explode('/', $menu['url_value']);
                    $menu['controller'] = $url[1];
                    $menu['action']     = $url[2];
                    $menu['url_value']  = $menu['url_type'] == 'module_shop' ? shop_url($menu['url_value'], $menu['params']) : home_url($menu['url_value'], $menu['params']);
                }
            }
    ...
    public static function getSidebarMenu($id = '', $module = '', $controller = '')
    {
        ...
        $cache_tag  = strtolower('_sidebar_menus_' . $module . '_' . $controller).'_role_'.session('shop_auth.role');
        $menus      = cache($cache_tag);
        if (!$menus) {
            // 获取当前节点地址
    ...
    public static function getLocation($id = '', $del_last_url = false, $check = true)
    {
        ...
        if (!$location) {
            ...
            if ($map['url_value']=='shop/index/index') {
                $map['url_value'] = 'shop/menu/index';
            }
            // 当前操作对应的节点ID
