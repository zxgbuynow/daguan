...
class Role extends Model
{
    ...
    public static function checkAuth($id = 0, $url = false)
    {
        ...
        // 当前用户的角色
        $role = session('shop_auth.role');
        // id为1的是超级管理员，或者角色为1的，拥有最高权限
        if (session('shop_auth.uid') == '1' || $role == '1') {
            return true;
        }

        // 获取当前用户的权限
        $menu_auth = session('shop_role_menu_auth');
    ...
    {
        $menu_auth = cache('shop_role_menu_auth_'.session('shop_auth.role'));
        if (!$menu_auth) {
            $menu_auth = self::where('id', session('shop_auth.role'))->value('menu_auth');
            $menu_auth = json_decode($menu_auth, true);
            $menu_auth = MenuModel::where('id', 'in', $menu_auth)->column('id,url_value');
        }
        // 非开发模式，缓存数据
        if (config('develop_mode') == 0) {
            cache('shop_role_menu_auth_'.session('shop_auth.role'), $menu_auth);
        }
        return $menu_auth;
    }
