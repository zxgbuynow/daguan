 ...
 */
class Clacategory extends Admin
{
    /**
     * 菜单列表
     * @param null $id 导航id
    ...
    public function index($id = null)
    {
        ...
        // 数据列表
        $data_list = Db::view('cms_clacategory', true)
            ->order('cms_clacategory.id')
            ->select();
    ...
    public function add($pid = 0)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($Category = ClacategoryModel::create($data)) {
                // 记录行为
                action_log('clacategory_add', 'cms_clacategory', $Category['id'], UID, $data['title']);
        ...
        return ZBuilder::make('form')
            ->addFormItems([
                ['hidden', 'pid', $pid],
                ['select', 'pid', '父类', '<code>必选</code>', ClacategoryModel::getTreeList(),$pid],
                ['text', 'title', '分类标题'],
                ['text', 'sort', '排序', '', 100],
                ['image', 'cover', '分类图片'],
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (ClacategoryModel::update($data)) {
                // 记录行为
                action_log('clacategory_edit', 'cms_clacategory', $id, UID, $data['title']);
        ...
        return ZBuilder::make('form')
            ->addFormItems([
                ['hidden', 'id'],
                ['select', 'pid', '父类', '<code>必选</code>', ClacategoryModel::getTreeList(0)],
                ['text', 'title', '分类标题'],
                ['text', 'sort', '排序', '', 100],
                ['image', 'cover', '分类图片'],
                ['radio', 'status', '立即启用', '', ['否', '是'], 1]
            ])
            ->setFormData(ClacategoryModel::get($id))
            ->fetch();
    }
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        $menu_title = ClacategoryModel::where('id', 'in', $ids)->column('title');
        return parent::setStatus($type, ['clacategory_'.$type, 'cms_clacategory', 0, UID, implode('、', $menu_title)]);
    }
    ...
    public function quickEdit($record = [])
    {
        ...
        $menu    = ClacategoryModel::where('id', $id)->value($field);
        $details = '字段(' . $field . ')，原值(' . $menu . ')，新值：(' . $value . ')';
        return parent::quickEdit(['claccategory_edit', 'cms_clacategory', $id, UID, $details]);
    }
