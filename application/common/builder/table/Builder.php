...
use app\shop\model\Menu as Smenu;
use app\common\builder\ZBuilder;
use app\user\model\Role;
use app\shop\model\Role as Srole;
use think\Cache;
...
class Builder extends ZBuilder
{
    ...
    public function autoAdd($items = [], $table = '', $validate = '', $auto_time = '', $format = '', $pop = false)
    {
        if (!empty($items)) {
            ...
            if ($this->_module=='shop') {
                if (session('shop_auth.role') != 1 && substr($btn_attribute['href'], 0, 4) != 'http' && $btn_attribute['href'] != 'javascript:history.back(-1);') {
    ...
    {
        if ($this->_module=='shop') {
            if (preg_match('/\/(index.php|shop.php)\/(.*)/', $btn_attribute['href'], $match)) {
                ...
                return Srole::checkAuth($url_value, true);
            }
        }else{
    ...
    public function autoEdit($items = [], $table = '', $validate = '', $auto_time = '', $format = '', $pop = false, $extra = [])
    {
        if (!empty($items)) {
            ...
            // 判断当前用户是否有权限，没有权限则不生成按钮
            if ($this->_module=='shop') {
                if (session('shop_auth.role') != 1 && substr($btn_attribute['href'], 0, 4) != 'http') {
                    if ($this->checkButtonAuth($btn_attribute) === false) {
                        return $this;
                    }
                }
