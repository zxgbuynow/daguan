...
class Index
{
    ...
    /**
     * [jssdk_custom description]
     * @param  [type] $params [description]
     * @return [type]         [description]
     */
    public function jssdk_custom($params)
    {
        $timestamp = time();
        $appId = 'wx9de3fea21b640cc9';
        $appsecret = '643732cb06a13a286fab6b36c5ae72c2';
    ...
    public function make_ticket($appId,$appsecret)
    {
        ...
        $data = json_decode(file_get_contents(ROOT_PATH."wxshare/access_token.json"));
        if (!is_dir(ROOT_PATH.'wxshare')) {
            mkdir(ROOT_PATH.'wxshare', 0755, true);
        }
        if ($data->expire_time < time()) {
            ...
            if ($access_token) {
                ...
                $fp = fopen(ROOT_PATH."wxshare/access_token.json", "w");
        ...
        $data = json_decode(file_get_contents(ROOT_PATH."wxshare/jsapi_ticket.json"));
        if ($data->expire_time < time()) {
            ...
            if ($ticket) {
                ...
                $fp = fopen(ROOT_PATH."wxshare/jsapi_ticket.json", "w");
                fwrite($fp, json_encode($data));
                fclose($fp);
            }
        }else{
