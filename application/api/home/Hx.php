...
class Hx 
{
    ...
    const CLIENT_ID='YXA6ONLG0GoiEeiN4hso3cS4sA';
    const CLIENT_SECRET='YXA6w0p9etRAbCdFDDOrEoBENHdRwXI';
    const ORG_NAME='1116180606099961';
    const APP_NAME='daguan';
    const URL = 'https://a1.easemob.com/1116180606099961/daguan/';
    ...
    /**
     * [getUserMsgCount 查询离线消息数]
     * @return [type] [description]
     */
    static public function getUserMsgCount($op)
    {
        $url = self::URL . "users/".$op.'/offline_msg_count';
    ...
    static public function getToken() {
        ...
        $option ['client_secret'] = self::CLIENT_SECRET;
        $url = self::URL . "token";
        $fp = @fopen ( "easemob.txt", 'r' );
        if ($fp) {
            $arr = unserialize ( fgets ( $fp ) );
            if ($arr ['expires_in'] < time ()) {
                $result = self::postCurl ( $url, $option );
     ...
     * CURL Post
     */
    static private function postCurl($url, $option, $header = [], $type = 'POST') {
